...
def formula_figure():
    ...
    ax.text(0.03, 0.12, "三维栅格分辨率: 1m × 1m × 1m；障碍物膨胀半径: 0.5m", fontsize=18, color="#004d40")
    fig.tight_layout()
...
def radar_figure():
    labels = ["路径效率", "安全裕量", "平滑性", "收敛速度", "工程可实现性"]
    ...
    ax.plot(angles, baseline, "--", linewidth=2, color="#ff7043", label="直线路径")
    ax.fill(angles, baseline, alpha=0.15, color="#ff7043")
    ax.plot(angles, improved, linewidth=2.5, color="#00acc1", label="改进ABC")
    ax.fill(angles, improved, alpha=0.22, color="#00acc1")
    ax.set_thetagrids([a * 180 / np.pi for a in angles[:-1]], labels)
    ax.set_title("巡检路径性能对比雷达图", pad=20)
