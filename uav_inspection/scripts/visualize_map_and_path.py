#!/usr/bin/env python3
"""离线可视化三维栅格地图与规划航迹（增强版）。"""

from __future__ import annotations

import json
from pathlib import Path

import numpy as np

try:
    import matplotlib
    import matplotlib.pyplot as plt
    from matplotlib import font_manager
except Exception:
    plt = None

ROOT = Path(__file__).resolve().parents[1]
MAP_DIR = ROOT / "artifacts" / "grid_map"
PLAN_DIR = ROOT / "artifacts" / "planning"
VIS_DIR = ROOT / "artifacts" / "visualization"
VIS_DIR.mkdir(parents=True, exist_ok=True)


def _setup_plot_font() -> bool:
    if plt is None:
        return False
    names = {f.name for f in font_manager.fontManager.ttflist}
    for n in ["Noto Sans CJK SC", "Noto Sans CJK JP", "WenQuanYi Micro Hei", "SimHei", "Microsoft YaHei"]:
        if n in names:
            matplotlib.rcParams["font.sans-serif"] = [n, "DejaVu Sans"]
            matplotlib.rcParams["axes.unicode_minus"] = False
            return True
    matplotlib.rcParams["font.sans-serif"] = ["DejaVu Sans"]
    matplotlib.rcParams["axes.unicode_minus"] = False
    return False


def main() -> None:
    if plt is None:
        print("[WARN] matplotlib 未安装，无法绘制可视化图。")
        print("[TIP] 安装: sudo apt install -y python3-matplotlib")
        return

    cjk = _setup_plot_font()

    occ = np.load(MAP_DIR / "voxel_map.npz")["occupancy"]
    with (MAP_DIR / "map_metadata.json").open("r", encoding="utf-8") as f:
        meta = json.load(f)
    with (PLAN_DIR / "planned_path.json").open("r", encoding="utf-8") as f:
        plan = json.load(f)

    raw = np.array(plan["raw_path"])
    smooth = np.array(plan["smooth_path"])
    x_min, x_max = meta["bounds"]["x"]
    y_min, y_max = meta["bounds"]["y"]
    z_min, z_max = meta["bounds"]["z"]
    res = meta["resolution"]

    occ_idx = np.argwhere(occ > 0)
    if len(occ_idx) > 18000:
        occ_idx = occ_idx[:: max(1, len(occ_idx) // 18000)]

    occ_world = np.zeros((len(occ_idx), 3), dtype=float)
    occ_world[:, 0] = x_min + (occ_idx[:, 0] + 0.5) * res
    occ_world[:, 1] = y_min + (occ_idx[:, 1] + 0.5) * res
    occ_world[:, 2] = z_min + (occ_idx[:, 2] + 0.5) * res

    start = np.array(meta["start"])
    targets = np.array(meta["targets"])

    fig = plt.figure(figsize=(13, 10), dpi=180)
    ax = fig.add_subplot(111, projection="3d")
    ax.scatter(occ_world[:, 0], occ_world[:, 1], occ_world[:, 2], s=3, c=occ_world[:, 2], cmap="plasma", alpha=0.20, label="障碍物")
    ax.plot(raw[:, 0], raw[:, 1], raw[:, 2], "--", lw=1.5, c="#ff7043", label="原始路径")
    ax.plot(smooth[:, 0], smooth[:, 1], smooth[:, 2], lw=2.8, c="#00e676", label="平滑路径")
    ax.scatter(start[0], start[1], start[2], c="#2979ff", s=150, marker="*", label="起点")
    ax.scatter(targets[:, 0], targets[:, 1], targets[:, 2], c="#ffd54f", s=36, label="巡检目标点")

    ax.set_xlim(x_min, x_max)
    ax.set_ylim(y_min, y_max)
    ax.set_zlim(z_min, z_max)
    ax.set_xlabel("X / m")
    ax.set_ylabel("Y / m")
    ax.set_zlabel("Z / m")
    ax.view_init(elev=28, azim=-58)
    ax.set_title("三维栅格地图 + 航迹" if cjk else "3D Voxel Map + Trajectory")
    ax.legend(loc="upper left")
    plt.tight_layout()
    fig.savefig(VIS_DIR / "map_and_path_3d.png")
    plt.close(fig)

    fig2, ax2 = plt.subplots(figsize=(11, 8), dpi=180)
    occ_xy = np.max(occ, axis=2).T
    ax2.imshow(occ_xy, origin="lower", extent=[x_min, x_max, y_min, y_max], cmap="magma", alpha=0.82)
    ax2.plot(raw[:, 0], raw[:, 1], "--", lw=1.5, c="#ff7043", label="原始路径")
    ax2.plot(smooth[:, 0], smooth[:, 1], lw=2.5, c="#00e676", label="平滑路径")
    ax2.scatter(start[0], start[1], c="#2979ff", s=130, marker="*", label="起点")
    ax2.scatter(targets[:, 0], targets[:, 1], c="#ffd54f", s=34, label="巡检目标")
    ax2.set_title("地图与航迹俯视图" if cjk else "Map & Trajectory Top View")
    ax2.set_xlabel("X / m")
    ax2.set_ylabel("Y / m")
    ax2.grid(alpha=0.2)
    ax2.legend(loc="upper right")
    plt.tight_layout()
    fig2.savefig(VIS_DIR / "map_and_path_xy.png")
    plt.close(fig2)

    print("[OK] 可视化图已生成:")
    print(" -", VIS_DIR / "map_and_path_3d.png")
    print(" -", VIS_DIR / "map_and_path_xy.png")


if __name__ == "__main__":
    main()
